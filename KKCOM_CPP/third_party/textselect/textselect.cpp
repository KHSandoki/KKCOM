...
static std::size_t getCharIndex(std::string_view s, float cursorPosX, std::size_t start, std::size_t end) {
    ...
    if (cursorPosX < widthToMidEx) {
        ...
        // Snap to the nearest caret edge: if the cursor is past this character's
        // horizontal midpoint, place the caret after it so the character under
        // the cursor is included in the selection. Without this the index always
        // rounds down, so the last character dragged over (e.g. a trailing
        // comma) never gets highlighted. [Local patch vs upstream v1.3.1]
        float charMid = (widthToMidEx + widthToMid) / 2.0f;
        return cursorPosX > charMid ? midIdx + 1 : midIdx;
    }
}
